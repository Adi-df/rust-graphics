...
const TRIANGLE_DISTANCE: f64 = 200.0;
...
fn main() {
    ...
    frame.draw({
        ...
        move |_this| {
            ...
            {
                let mut path_points =
                    sort_by_distance((0, 0), &points.iter().map(|p| p.0).collect())
                        .into_iter()
                        .map(|(p, _)| (p, false))
                        .collect::<Vec<(Point, bool)>>();
                path_points[0].1 = true;
                let mut path = vec![path_points[0].0];
                loop {
                    let nearests = sort_by_distance(
                        path.last().unwrap().clone(),
                        &path_points
                            .iter()
                            .filter_map(|p| if p.1 { None } else { Some(p.0) })
                            .collect(),
                    );
                    if nearests.len() == 0 {
                        break;
                    }
                    path.push(nearests[0].0);
                    path_points.iter_mut().for_each(|p| {
                        if p.0 == nearests[0].0 {
                            p.1 = true
                        }
                    })
                ...
                while let Some(p) = path_iter.next() {
                    if let Some(p2) = path_iter.peek() {
                        draw::draw_line(p.0, p.1, p2.0, p2.1)
                    }
                }
            }
        }
